...
export const SectionCover: React.FC<SectionCoverProps> = ({
  ...
  const periodText = data.course.endDate && data.course.endDate.trim() !== ''
    ...
    : `Início em ${data.course.startDate || '22/05/2026'}`;
      ...
      <div className="relative h-64 md:h-80 w-full overflow-hidden">
        ...
        <div className="absolute top-6 left-6 right-6 flex flex-wrap justify-between items-center gap-3 text-white">
          <div className="flex items-center space-x-2 bg-slate-900/80 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/10 shadow-lg">
            ...
            <span className="text-xs font-semibold tracking-wider uppercase">E-Portefólio de Formando</span>
          </div>

          <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/20 text-xs font-medium">
      ...
      <div className="relative max-w-6xl mx-auto px-6 sm:px-8 pb-10 z-10">
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="relative group">
            ...
            <div className="absolute -bottom-2 -right-2 bg-emerald-500 text-white p-1.5 rounded-full shadow-lg border-2 border-white dark:border-slate-900" title="Formando Ativo">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
              <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold tracking-wide ${palette.badgeBg}`}>
                <Award className="w-3.5 h-3.5" />
                Ação {data.course.actionNumber}
              </span>
              <span className="text-xs font-medium px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
          ...
          <div className="w-full md:w-auto bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 flex flex-col gap-2 text-xs text-slate-600 dark:text-slate-300">
            <div className="flex items-center gap-2">
              <Calendar className={`w-4 h-4 ${palette.primaryText}`} />
              <span className="font-semibold">Período de Formação:</span>
            </div>
