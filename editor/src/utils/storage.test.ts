...
test('um projeto novo começa neutro com uma UFCD', () => {
  const blank = createBlankTemplate();
  assert.equal(blank.course.actionNumber, '');
  assert.equal(blank.course.actionTitle, '');
  assert.equal(blank.ufcds.length, 1);
  assert.equal(blank.ufcds[0].code, '');
...
test('ações e UFCD próprias sobrevivem à gravação e ao carregamento', () => {
  ...
  const custom = {
    ...
    ufcds: [
      { ...blank.ufcds[0], id: 'ufcd_a', code: '9001', name: 'Primeira', hours: 20 },
      { ...blank.ufcds[0], id: 'ufcd_b', code: '9002', name: 'Segunda', hours: 30 },
    ],
  };
  ...
  assert.equal(loaded.templateId, 'eportfolio_generico');
});
